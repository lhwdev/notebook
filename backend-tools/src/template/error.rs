...
pub(crate) fn lookup_error(source: &str, step: &str, path: PathSlice, context: &Ctx) -> Error {
    let avail_str = if let map = context.value_tree().list_nested(&path[0..path.len() - 1]) {
